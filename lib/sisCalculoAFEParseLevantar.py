...
class ParseLevantar(object):
	...
	def __init__(self):
		...
		for index, row in PagosRetenidos.iterrows():
			...
			if Bis.empty:
				cnx.execute("UPDATE PagosRetenidos SET '80Bis'=? WHERE unico=?", ("" , row['unico']))
				cnx.commit()
			else:
				cnx.execute("UPDATE PagosRetenidos SET '80Bis'=? WHERE unico=?", (int(diff80bis[0]) , row['unico']))
				cnx.commit()
			# ===================================================================f ====================================================================================
			queryRU  = " \
				SELECT \
			...
			if RU.empty:
				cnx.execute("UPDATE PagosRetenidos SET 'Urgencias'=? WHERE unico=?", ("" , row['unico']))
				cnx.commit()
			else:
				cnx.execute("UPDATE PagosRetenidos SET 'Urgencias'=? WHERE unico=?", (int(diffRU[0]) , row['unico']))
				cnx.commit()
			# =======================================================================================================================================================
		...
		PagosRetenidos.to_excel(writer, sheet_name='Pagos Retenidos', index=False)

		# Obtener el objeto workbook y la hoja de trabajo
		workbook = writer.book
		worksheet = writer.sheets['Pagos Retenidos']
