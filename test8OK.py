	...
	def databaseArbol(self, row):
		...
		consulta = " \
			...
			WHERE \
				...
				AND c.'cuenta' = r.'cuenta' \
				AND c.'tipo_de_pago' = r.'tipo_de_pago' \
				AND c.'tipo_de_pago' <> '80 BIS' \
		"
		df5 = pd.read_sql_query(consulta, cnx)
		df5agrupado = df5.groupby(["Unidad Ejecutora", "cuenta", "FolioNacional", "DisponibleNacional", "tipo_de_pago", "r.Folio", "r.MontoVigente", "r.MontoDisponible", "r.MontoConsumido", "c.rut", "c.Folio", "c.MontoDisponible"]).sum().reset_index()

		df5_pivot_table = pd.pivot_table(df5,
							index = ["Unidad Ejecutora", "cuenta", "FolioNacional", "DisponibleNacional", "tipo_de_pago", "r.Folio", "r.MontoVigente", "r.MontoDisponible", "r.MontoConsumido", "c.rut", "c.Folio", "c.MontoDisponible"],
							#values = ["monto"],
							#columns = ["proyecto"],
							aggfunc = [np.sum],
							fill_value = 0,
		...
		print(df5_pivot_table.columns)
		df5_pivot_table.columns = df5_pivot_table.columns.droplevel()
		# ----------------------------------------------------------------------------------------------

		today = date.today()
		writer = pd.ExcelWriter(today.strftime("output/"+"%d-%b-%Y")+' TEST 8 - '+str(row['Unidad Ejecutora'])+'.xlsx', engine='xlsxwriter')
		...
		df5agrupado.to_excel(writer, sheet_name="df5agrupado")
		df5_pivot_table.to_excel(writer, sheet_name="df5_pivot_table")
		writer.save()
