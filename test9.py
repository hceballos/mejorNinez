...
class main(object):
	def __init__(self):
		...
		disponible = pd.pivot_table(df4,
							index = ["Unidad Ejecutora", "cuenta", "FolioNacional", "DisponibleNacional", "tipo_de_pago", "r.Folio", "r.MontoDisponible", "c.rut", "c.Folio", "c.MontoDisponible", "d.MontoDisponible"],
							#values = ["monto"],
							#columns = ["proyecto"],
							aggfunc = [np.sum],
							fill_value = 0,
		...
		today = date.today()
		writer = pd.ExcelWriter(today.strftime("output/"+"%d-%b-%Y")+' TEST 9 - centralizacion.xlsx', engine='xlsxwriter')
