...
class main(object):
	def __init__(self):
		...
		disponible = pd.pivot_table(df4,
							index = ["Unidad Ejecutora", "cuenta", "FolioNacional", "DisponibleNacional", "tipo_de_pago", "r.Folio", "r.MontoVigente", "r.MontoDisponible", "r.MontoConsumido", "c.rut", "c.Folio", "c.MontoVigente", "c.MontoDisponible", "c.MontoConsumido", "d.Folio",  "d.mes" , "d.MontoVigente", "d.MontoDisponible", "d.MontoConsumido"],
							#values = ["monto"],
							#columns = ["proyecto"],
							aggfunc = [np.sum],
							fill_value = 0,
		...
		today = date.today()
		writer = pd.ExcelWriter(today.strftime("output/"+"%d-%b-%Y")+' TEST 9v2 - centralizacion.xlsx', engine='xlsxwriter')
